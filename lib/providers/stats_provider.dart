    ...
    try {
      ...
      final results = await Future.wait(
        activeClients.map((client) async {
          final clientTorrents = allTorrents.where(
            (t) => t.clientId == client.id,
          );
          // 客户端在线状态：主动测试结果 或 能获取到种子（说明连通）
          final clientOnline =
              (onlineStatus[client.id] ?? false) || clientTorrents.isNotEmpty;
          int clientDl = 0;
          int clientUl = 0;
          int clientSize = 0;
          ...
          final totalDl = clientTorrents.fold<int>(
            0,
            (sum, t) => sum + t.downloaded,
          );
          final totalUl = clientTorrents.fold<int>(
            0,
            (sum, t) => sum + t.uploaded,
          );
